...
`niuma` 不是 AI agent。它是一个桌面连接器，让 Niuma iOS app 可以通过启动或
连接 `codex app-server` 来使用 Codex。因此启动 Niuma Gateway 前，需要先
安装 Codex.app 或 Codex CLI。

## 安装

1. 安装 Codex.app 或 Codex CLI。
2. 从 crates.io 安装桌面 Gateway：
