...
2. iOS app 目前仍处于测试阶段。请通过 Xcode 自行编译到 iPhone，或发送邮件到
...
iOS app 是原生 SwiftUI 项目，目前仍处于测试阶段。需要在 iPhone 上使用时，请
通过 Xcode 选择真机后自行编译运行，或发送邮件到 `rainchestnut@gmail.com`
获取 TestFlight 测试链接。源码开发和模拟器检查时使用：
